...
_FABRIC_REQUIRED_VARS: list[str] = [
    "AZURE_TENANT_ID",
    "AZURE_APP_CLIENT_ID",
    "KEY_VAULT_URL",
...
_FABRIC_ENV_DESCRIPTIONS: dict[str, str] = {
...
def _fabric_auth_configured() -> bool:
    """Check whether all Fabric Key Vault authentication variables are set.

    Returns:
        bool: ``True`` if every variable in ``_FABRIC_REQUIRED_VARS`` is
            present and non-empty in the environment.
    """
    return all(os.getenv(name) for name in _FABRIC_REQUIRED_VARS)


def _log_fabric_environment_info() -> None:
    ...
    for var_name, description in _FABRIC_ENV_DESCRIPTIONS.items():
...
def _build_fabric_token_provider() -> Callable[[], str]:
    ...
    """
    env_values = {name: os.getenv(name, "") for name in _FABRIC_REQUIRED_VARS}
...
def _build_missing_credentials_error(
    ...
    if _is_fabric_environment():
        lines.append("")
        lines.append("Fabric environment detected — Key Vault authentication also requires:")
        fabric_vars = [
            ("AZURE_TENANT_ID", os.getenv("AZURE_TENANT_ID"), '"your-tenant-id"'),
            ("AZURE_APP_CLIENT_ID", os.getenv("AZURE_APP_CLIENT_ID"), '"your-client-id"'),
            ("KEY_VAULT_URL", os.getenv("KEY_VAULT_URL"), '"https://YOUR-KEYVAULT.vault.azure.net/"'),
            ("KEY_VAULT_SECRET_NAME", os.getenv("KEY_VAULT_SECRET_NAME"), '"your-secret-name"'),
        ]
        for var_name, var_value, example in fabric_vars:
            if var_value:
                lines.append(f"  ✓ {var_name} is set")
            else:
                lines.append(f"  ✗ {var_name} is not set")
...
def _register_default_providers() -> None:
    ...
    if _is_fabric_environment():
        _log_fabric_environment_info()

    if _is_fabric_environment() and _fabric_auth_configured():
        CONTAINER.register(
            BearerTokenProvider,
            lambda: BearerTokenProvider(value=_build_fabric_token_provider()),
        )
    else:
