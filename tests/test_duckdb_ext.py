...
class TestImpute:
    def test_impute_returns_relation_with_no_nulls(self):
        """Test impute with a mock that avoids API calls."""
        from unittest.mock import patch

        from openaivec.duckdb_ext import impute
        from openaivec.task.table.fillna import FillNaResponse

        conn = duckdb.connect(":memory:")
        conn.execute("""
            CREATE TABLE employees (name VARCHAR, dept VARCHAR, salary INTEGER);
            INSERT INTO employees VALUES
                ('Alice', 'Engineering', 100),
                ('Bob', 'Sales', 200),
                ('Charlie', 'Engineering', NULL),
                ('Diana', 'Sales', NULL);
        """)

        mock_results = [
            FillNaResponse(index=0, output=150),
            FillNaResponse(index=1, output=180),
        ]

        with patch("openaivec.duckdb_ext.run_async", return_value=mock_results):
    ...
    def test_impute_no_nulls_returns_unchanged(self):
        from openaivec.duckdb_ext import impute

        conn = duckdb.connect(":memory:")
        conn.execute("""
            CREATE TABLE full_data (x INTEGER, y INTEGER);
            INSERT INTO full_data VALUES (1, 10), (2, 20);
        """)

        result = impute(conn, "full_data", "y")
        df = result.df()
        assert len(df) == 2
